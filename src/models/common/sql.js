...
const reqSELECTBYID = async (params, dbTableDef) => {
    let sqlWhereClause = ''
    const reqParams = []
...
/*********************************************************
Requête DELETE BY ID (method DELETE)
*********************************************************/

const reqDELETEBYID = async (params, dbTableDef) => {
    let sqlWhereClause = ''
    const reqParams = []
    ...
    if (pathParameter) {
        ...
        return {success: false, code: 400, error: `Pas de 'Path Parameter' (checkPathParameter)`}
    }

    // Construction de la requête SQL
    const reqTables = dbTableDef.tableName

    const reqSql = `DELETE FROM ${reqTables}${sqlWhereClause}`
console.log('SQL -> ', reqSql)
    // Éxecution de la requête SQL, envoi du résultat au contrôleur
    let conn
    try {
        conn = await db.getConnection()
        const result = await conn.query(reqSql, reqParams)
...
module.exports = {reqSELECT, reqSELECTBYID, reqDELETEBYID}